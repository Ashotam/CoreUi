...
function Dashboard(props) {
    ...
    const serchedData = (()=>{
      ...
      if(studentName){
        serchedData= serchedData.filter(student=>student.name.toLowerCase().includes(studentName.toLowerCase())===true)
       }
       if(studentLastName){
        serchedData= serchedData.filter(student=>student.lastName.toLowerCase().includes(studentLastName.toLowerCase())===true)
       }
       if(studentEmail){
        serchedData= serchedData.filter(student=>student.email.toLowerCase().includes(studentEmail.toLowerCase())===true)
       }
       if(studentPhone){
        serchedData= serchedData.filter(student=>student.phone.toLowerCase().includes(studentPhone.toLowerCase())===true)
       }
       if(selectedFac){
        serchedData= serchedData.filter(student=>student.faculty===selectedFac)
       }
       if(selectedGroup){
        serchedData= serchedData.filter(student=>student.group===selectedGroup)
       }
      return serchedData
    })()
            ...
            <Card>
              ...
              <CardBody>
                <Table responsive>
                  ...
                  <tbody>
                    <tr>
                    <td className="pl-1"><Input type="text"  placeholder="Search by Id..." name="id" value = {studentId} onChange={(e)=>{setStudentId(e.target.value)}} /></td>
                    <td className="pl-1"><Input type="text"  placeholder="Search by Name..." name="name" value = {studentName} onChange={(e)=>{setStudentName(e.target.value)}} /></td>
                    ...
                    <td className="pl-1"><Input type="select" name="selectFaculty" id="selectFaculty" placeholder={""} onChange = {(e)=>{selectFaculty(e)}}>
                      ...
                      {serchedData&&serchedData.map(student=>
                      <tr key= {student.id}>
                      <td >{student.id}</td>
                      <td >{student.name} </td>
