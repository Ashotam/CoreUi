...
function Facultets(props) {
  ...
  const delateHandler = (id) => {
    ...
    if (deletedFac.groups.length === 0) {
      if (facData) setSelectedFac(id);
      setIsOpenModal(true);
    } else {
      return; /* notifaciton dnel*/
    }
  };
  const onDelete = () => {
    ...
    <div>
      {!isOpenedCreate && !isopenedEdit && (
        <Row>
          ...
          <Col>
            <Card>
              <CardHeader>Faculty Table</CardHeader>
              ...
              <CardBody>
                ...
                >
                  <Button color="success" onClick={openCreateFaculty}>
                    Create Faculty
                  </Button>
                </Col>
                <Table responsive>
...
export default connect(mapStateToProps, mapDispatchToProps)(Facultets);
